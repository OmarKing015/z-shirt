...
export default function AdminPanel() {
  ...
  const uploadLogo = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!logoForm.file || !logoForm.name || !logoForm.category) return

    setUploadingLogo(true)
    const formData = new FormData()
    formData.append("file", logoForm.file)
    formData.append("name", logoForm.name)
    formData.append("category", logoForm.category)
    ...
    try {
      ...
      setLogoForm({ name: "", category: "", file: null })
      fetchData()
    } catch (error) {
      toast({
        title: "Error",
  ...
  const uploadColorSwatch = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!swatchForm.file || !swatchForm.name || !swatchForm.hexCode) return
    ...
    try {
      ...
      setSwatchForm({ name: "", hexCode: "", file: null })
      fetchData()
    } catch (error) {
      toast({
        title: "Error",
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="bg-blue-500 text-white p-6">
        ...
        <p className="text-blue-100 mt-2">Manage files, logos, and color swatches</p>
      </div>

      <div className="container mx-auto p-6">
        <Tabs defaultValue="files" className="w-full">
          <TabsList className="grid w-full grid-cols-2 bg-blue-50">
            <TabsTrigger value="files" className="data-[state=active]:bg-blue-500 data-[state=active]:text-white">
          ...
          <TabsContent value="files" className="mt-6">
            <Card>
              <CardHeader>
                <CardTitle className="text-blue-500">Zipped Material Files</CardTitle>
                <CardDescription>Manage and download zipped files uploaded by users</CardDescription>
              </CardHeader>
              <CardContent>
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow className="bg-blue-50">
                        <TableHead className="text-blue-700">Order Number</TableHead>
                        <TableHead className="text-blue-700">User Name</TableHead>
                        <TableHead className="text-blue-700">File Name</TableHead>
                        <TableHead className="text-blue-700">Upload Date</TableHead>
                        <TableHead className="text-blue-700">File Size</TableHead>
                    ...
                    <TableBody>
                      {(zippedFiles||[])?.map((file) => (
                        <TableRow key={file._id}>
                          <TableCell className="font-medium">{file.orderNumber}</TableCell>
                          ...
                          <TableCell>
                            <Badge
                              ...
                              className={file.status === "completed" ? "bg-blue-500" : ""}
                            >
                              {file.status}
                            </Badge>
                          </TableCell>
              ...
              <Card>
                ...
                <CardContent>
                  <form onSubmit={uploadLogo} className="space-y-4">
                    ...
                    <div>
                      <Label htmlFor="logo-category">Category</Label>
                      <Input
                        id="logo-category"
                        value={logoForm.category}
                        onChange={(e) => setLogoForm({ ...logoForm, category: e.target.value })}
                  ...
                  <form onSubmit={uploadColorSwatch} className="space-y-4">
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => setSwatchForm({ ...swatchForm, hexCode: e.target.value })}
                        placeholder="#000000"
                        className="border-blue-200 focus:border-blue-500"
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        ...
                        type="file"
                        accept="image/*"
                        onChange={(e) => setSwatchForm({ ...swatchForm, file: e.target.files?.[0] || null })}
                        className="border-blue-200 focus:border-blue-500"
                      />
                    </div>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="grid grid-cols-2 gap-4">
                    {logos.map((logo) => (
                      <div key={logo._id} className="border border-blue-200 rounded-lg p-3">
                ...
                <CardContent>
                  <div className="grid grid-cols-2 gap-4">
                    {colorSwatches.map((swatch) => (
                      <div key={swatch._id} className="border border-blue-200 rounded-lg p-3">
                        <div className="aspect-square relative mb-2">
                          <Image
                        ...
                        <p className="text-xs text-gray-500">{swatch.hexCode}</p>
                        <Button
                          size="sm"
                          variant="destructive"
